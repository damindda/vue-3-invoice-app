...
store.dispatch('SET_CURRENT_INVOICES', route.params.invoiceId)

console.log(state.invoiceModal.currentInvoices)
...
const toggleEditInvoice = () => {
    ...
    store.dispatch('SET_TOGGLE_EDIT_INVOICE')
    return {
        toggleEditInvoice,
    }
}
...
const deleteInvoice = async (docId) => {
    await store.dispatch('SET_DELETE_INVOICE', docId)
...
const updateStatusToPaid = (docId) => {
    store.dispatch('SET_UPDATE_STATUS_TO_PAID', docId)
...
const updateStatusToPending = (docId) => {
    store.dispatch('SET_UPDATE_STATUS_TO_PENDING', docId)
...
.green {
    background-color: rgb(4, 151, 119);
}
.invoice-view {
    flex: 1;
}
